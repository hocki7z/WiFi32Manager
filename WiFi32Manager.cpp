/**************************************************************
  WiFiManager32 is a library for the ESP32 platform
...
boolean  WiFiManager::startConfigPortal(char const *apName, char const *apPassword) {
  ...
  if (connRes == WL_CONNECTED){
    ...
    // When ESP station is trying to find a target AP, it will scan on every channel,
    // that means ESP station is changing its channel to scan. This makes the channel of ESP softAP keep changing too..
...
int WiFiManager::connectWifi(String ssid, String pass) {
  ...
  if (ssid != "") {
    ...
    esp will sometimes lock up when SSID or password is different to
